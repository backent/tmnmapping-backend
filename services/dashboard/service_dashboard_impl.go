...
func (s *ServiceDashboardImpl) buildReport(ctx context.Context, table, dedupField, pic, dateFrom, dateTo string) webDashboard.DashboardReport {
	tx, err := s.DB.Begin()
	helpers.PanicIfError(err)
	defer helpers.CommitOrRollback(tx)

	// 1. Status counts
	statusCounts, err := s.Repository.GetStatusCounts(ctx, tx, table, dedupField, pic, dateFrom, dateTo)
	helpers.PanicIfError(err)
	...
	personTypeRows, err := s.Repository.GetByPersonAndType(ctx, tx, table, dedupField, pic, dateFrom, dateTo)
	helpers.PanicIfError(err)
	...
	personStatusRows, err := s.Repository.GetByPersonAndStatus(ctx, tx, table, dedupField, pic, dateFrom, dateTo)
	helpers.PanicIfError(err)
