...
type ServicePOIInterface interface {
	...
	FindAll(ctx context.Context, request webPOI.POIRequestFindAll) ([]webPOI.POIResponse, int)
	...
	Export(ctx context.Context, search string, categoryIds string, subCategoryIds string, motherBrandIds string) ([]byte, error)
}
