...
func (r *RepositorySubCategoryImpl) FindAll(ctx context.Context, tx *sql.Tx, take int, skip int, orderBy string, orderDirection string, search string) ([]models.SubCategory, error) {
	...
	var err error
	if search != "" {
		SQL := `SELECT id, name, created_at, updated_at FROM ` + models.SubCategoryTable + ` WHERE name ILIKE $1 ORDER BY ` + orderBy + ` ` + orderDirection + ` LIMIT $2 OFFSET $3`
		rows, err = tx.QueryContext(ctx, SQL, "%"+search+"%", take, skip)
	} else {
		SQL := `SELECT id, name, created_at, updated_at FROM ` + models.SubCategoryTable + ` ORDER BY ` + orderBy + ` ` + orderDirection + ` LIMIT $1 OFFSET $2`
